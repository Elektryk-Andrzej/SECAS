...
class ActionHandler:
    ...
    async def HINTPLAYER(self) -> bool:
        if not await self.param_handler.is_action_required_len(3, None):
            return False

        elif not await self.param_handler.is_param_se_var(1):
            return False

        elif not await self.param_handler.is_param_number(2, float):
            return False
    ...
    async def COUNTDOWN(self) -> bool:
        ...
        elif not await self.param_handler.is_param_number(2, int):
            return False
    ...
    async def CLEARCASSIE(self) -> bool:
        if not await self.param_handler.is_action_required_len(0, 0):
            return False
    ...
    async def REMOVEITEM(self) -> bool:
        if not await self.param_handler.is_action_required_len(2, 3):
            ...
            return False
        if not await self.param_handler.is_param_special_var(2, var_type=self.data.item_type):
            return False
    ...
    async def LIGHTCOLOR(self) -> bool:
        if not await self.param_handler.is_action_required_len(4, 4):
            return False
        if not await self.param_handler.is_param_special_var(1, var_type=self.data.room_type, star_allowed=True):
            return False
        for index in range(2, 5):
            if not await self.param_handler.is_param_number(index, int, min_value=0, max_value=255):
                return False
    ...
    async def GOTO(self) -> bool:
        if not await self.param_handler.is_action_required_len(1, 1):
            return False
        if not await self.param_handler.is_param_label(1):
            return False
    ...
    async def GOTOIF(self) -> bool:
        ...
        if not await self.param_handler.is_param_label(1):
            return False
        if not await self.param_handler.is_param_label(2):
            return False
    ...
    async def DAMAGE(self) -> bool:
        ...
        if not await self.param_handler.is_param_se_var(1):
            return False
        if not await self.param_handler.is_param_number(2, float, required=False):
            return False
    ...
    async def RADIORANGE(self) -> bool:
        if not await self.param_handler.is_action_required_len(3, 3):
            return False
    ...
    async def AHP(self) -> bool:
        ...
        if not self.param_handler.is_param_number(2, float):
            return False
    ...
    async def TPX(self) -> bool:
        ...
        for i in range(2, 5):
            if not await self.param_handler.is_param_number(i, float):
                return False

        return True

    async def SIZE(self) -> bool:
        if not await self.param_handler.is_action_required_len(4, 5):
            return False
        ...
        if not await self.param_handler.is_param_number(5, int, required=False):
            return False

        return True

    async def EFFECT(self) -> bool:
        if not await self.param_handler.is_action_required_len(3, 5):
            return False
    ...
    async def ADVSETROLE(self) -> bool:
        if not await self.param_handler.is_action_required_len(2, 5):
            return False
        ...
        if not await self.param_handler.is_param_bool(4, required=False):
            return False
    ...
    async def ROUNDLOCK(self) -> bool:
        if not await self.param_handler.is_action_required_len(1, 1):
            return False
        if not await self.param_handler.is_param_bool(1):
            return False
    ...
    async def SAVEPLAYERS(self) -> bool:
        ...
        if not await self.param_handler.register_variable(1, 2, player_var=True):
            return False
    ...
    async def SAVE(self) -> bool:
        if not await self.param_handler.is_action_required_len(1, None):
            return False
        if not await self.param_handler.register_variable(1, 2,
    ...
    async def WAITSEC(self) -> bool:
        if not await self.param_handler.is_action_required_len(1, None):
            return False
        if not await self.param_handler.is_param_number(1, float, math_supported=True):
            return False
    ...
    async def ADVAHP(self) -> bool:
        if not await self.param_handler.is_action_required_len(2, 7):
            return False
        ...
        if not await self.param_handler.is_param_number(3, float, required=False):
            return False

        if not await self.param_handler.is_param_number(4, float, required=False):
            return False

        if not await self.param_handler.is_param_number(5, float, required=False):
            return False

        if not await self.param_handler.is_param_number(6, float, required=False):
            return False

        if not await self.param_handler.is_param_bool(7, required=False):
            return False
